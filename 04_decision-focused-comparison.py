...
def update_theta_vjp(
    ...
    final_grad_products = torch.autograd.grad(outputs=grad_theta, inputs=[p_alpha_param, r_theta_param], grad_outputs=grad_true, retain_graph=False, allow_unused=True)
...
# -------------------------
def update_q_table_inplace_for_acting(q_table, target_q_table, probs_theta, rewards_theta, replay, inner_lr, K, gamma):
...
def train_omd(
    ...
    for it in range(max_iterations):
        ...
        update_q_table_inplace_for_acting(q_table, target_q_table, probs_theta, rewards_theta, replay, inner_lr, K, gamma)
        ...
        # outer update
        update_theta_vjp(rewards_theta, probs_theta, list(range(S)), list(range(A)), q_inner, target_q_table, gamma, replay, n_meta_iterations, meta_lr, device)
...
# -------------------------
def batch_run(env_name="FrozenLake-v1", map_name="4x4", is_slippery=True, n_seeds=30, max_iterations=1200, eval_episodes=500):
    rows = []
    for i in range(n_seeds):
        seed = 1000 + i
        env = gym.make(env_name, map_name=map_name, is_slippery=is_slippery)
        learned_P, learned_R, replay = train_omd(env, seed, max_iterations=max_iterations)
        ...
        mean_learn, std_learn = evaluate_policy(env, pi_learned, n_episodes=eval_episodes, seed=seed)
        mean_emp, std_emp = evaluate_policy(env, pi_emp, n_episodes=eval_episodes, seed=seed)
        mean_true, std_true = (None, None)
        if pi_true is not None:
            mean_true, std_true = evaluate_policy(env, pi_true, n_episodes=eval_episodes, seed=seed)
        ...
        env.close()
        print(f"Finished seed {i+1}/{n_seeds}: mean_learn={mean_learn:.4f}, mean_emp={mean_emp:.4f}, replay={len(replay)}")
...
if config["re-run"]:
    df = batch_run(env_name="FrozenLake-v1", map_name="4x4", is_slippery=True, n_seeds=30, max_iterations=1200, eval_episodes=500)
    print("\nSUMMARY")
    print(df.describe())
...
print(f"mean difference = {mean_diff:.6f} ± {t_crit * se_diff:.6f} (95% CI = [{ci_lower:.6f}, {ci_upper:.6f}])")

# simple boxplot + dots
plt.figure(figsize=(6, 4))
sns.set_style("white")
ax = sns.boxplot(data=[learned_means, empirical_means], palette=["#4c72b0", "#dd8452"], showfliers=False)
sns.stripplot(data=[learned_means, empirical_means], color="k", size=6, jitter=0.12, dodge=True, alpha=0.8)
p_text = f"paired t-test: p = {p_value:.3g}"
md_text = f"mean diff = {mean_diff:.4f}"
plt.text(0.5, max(np.max(learned_means), np.max(empirical_means)) * 1.05, p_text + "  |  " + md_text, ha="center", va="bottom", fontsize=10)
plt.xticks([0, 1], ["learned_means", "empirical_means"])
plt.ylabel("Mean episodic return")
plt.tight_layout()
plt.savefig(os.path.join(out_dir, "boxplot.png"), dpi=300)
